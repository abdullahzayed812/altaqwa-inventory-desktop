...
export const ReportsPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchReportData = async () => {
      try {
        const reportData = await (window as any).api.reports.getReportData();
        setData(reportData);
      } catch (error) {
    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex gap-3">
          <button 
            onClick={handlePrint}
            className="flex items-center gap-2 border border-slate-200 text-slate-600 px-6 py-2.5 rounded-xl text-label-md hover:bg-slate-50 transition-all"
          >
      ...
      <div className="grid grid-cols-12 gap-6">
            ...
            {data.recentSales.map((s: any, i: number) => {
              ...
              return (
                <div key={i} className="w-full bg-slate-50 rounded-t-lg relative group h-full">
                  <div
                    className="absolute bottom-0 w-full bg-primary/60 rounded-t-lg transition-all duration-500"
        ...
        {/* Profit Summary Card (Placeholder for now) */}
              ...
              <tbody className="text-body-md divide-y divide-slate-50">
                {data.customerBalances.map((customer: any) => (
                  <tr key={customer.id} className="hover:bg-blue-50/50 transition-colors">
                    ...
                    <td className="py-4">
                      <span
                        className={`px-2 py-1 text-label-sm rounded-lg ${Number(customer.totalDebt) > 0 ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`}
                      >
