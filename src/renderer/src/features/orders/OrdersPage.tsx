...
export const OrdersPage: React.FC = () => {
  ...
  const loadOrders = async () => {
    try {
      // @ts-ignore
      const data = await window.api.orders.getOrders();
      setOrders(data);
    } catch (e) {
  ...
  const handleStatusUpdate = async (orderId: number, newStatus: string) => {
    ...
    if (!window.confirm(`هل أنت متأكد من تغيير حالة الطلب إلى "${statusText}"؟`)) {
      return;
    }
      ...
      <div className="flex items-end justify-between mb-8">
        ...
        <div className="flex gap-3">
          <button
            ...
            className="bg-white text-slate-700 px-6 py-3 rounded-xl font-bold flex items-center gap-2 border border-slate-200 hover:bg-slate-50 transition-all shadow-sm"
          >
            ...
            {orders.length === 0 ? (
              ...
              orders.map((order) => (
                <tr key={order.id} className={`hover:bg-blue-50/30 transition-colors group ${order.status === 'CANCELLED' ? 'opacity-60 bg-slate-50/50' : ''}`}>
                  ...
                  <td className="px-6 py-4 text-data-tabular font-bold text-center">{Number(order.totalAmount).toLocaleString()} ج.م</td>
