...
export const PaymentsPage: React.FC = () => {
  ...
  const fetchPayments = async () => {
    setLoading(true);
    try {
      const cData = await (window as any).api.payments.getPayments();
      const sData = await (window as any).api.suppliers.getAllPayments();
    ...
    <div className="p-8 rtl">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex gap-3">
          <button
            onClick={handlePrint}
            className="flex items-center gap-2 bg-white text-slate-700 px-6 py-3 rounded-xl font-label-md border border-slate-200 hover:bg-slate-50 transition-all shadow-sm"
          >
          ...
          {activeTab === "customers" ? (
            <button
                ...
                className="flex items-center gap-2 bg-primary text-white px-6 py-3 rounded-xl font-label-md hover:opacity-90 transition-all shadow-lg"
            >
            ...
            <button
                ...
                className="flex items-center gap-2 bg-emerald-600 text-white px-6 py-3 rounded-xl font-label-md hover:bg-emerald-700 transition-all shadow-lg"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 bg-white p-4 rounded-xl border border-slate-100 shadow-sm">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder={activeTab === "customers" ? "بحث باسم العميل..." : "بحث باسم المورد..."}
            value={searchName}
