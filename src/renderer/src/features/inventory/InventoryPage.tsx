...
export const InventoryPage: React.FC = () => {
  ...
  const loadProducts = async () => {
    try {
      // @ts-ignore
      const data = await window.api.inventory.getProducts();
      setProducts(data);
    } catch (e) {
    ...
    <div className="px-8 pb-12 rtl">
      <div className="flex justify-between items-end mb-8">
        ...
        <div className="flex gap-3">
          <button
            onClick={handlePrint}
            className="flex items-center gap-2 px-4 py-2 border border-slate-200 bg-white rounded-lg text-slate-700 font-label-md hover:bg-slate-50 transition-all"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-lg font-label-md hover:opacity-90 transition-all"
          >
              ...
              {products.length === 0 ? (
                <tr>
                  <td colSpan={5} className="text-center py-8 text-slate-400">
                    لا يوجد منتجات حالياً. أضف منتج جديد.
                  </td>
                </tr>
              ) : (
