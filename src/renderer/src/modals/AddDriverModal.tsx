...
export const AddDriverModal: React.FC<AddDriverModalProps> = ({ isOpen, onClose, onSuccess }) => {
    ...
    const handleClose = () => {
        setName(''); setPhone(''); setVehiclePlate(''); setVehicleDetails('');
        setBalanceAmount(''); setBalanceType('دائن');
        onClose();
    };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm rtl">
            <div className="bg-white w-full max-w-lg rounded-xl shadow-2xl border border-slate-100 overflow-hidden">
                ...
                <div className="px-8 py-8 space-y-5">
                    <form id="add-driver-form" onSubmit={handleSubmit} className="space-y-5">
                        <div className="space-y-2">
                            ...
                            <input value={name} onChange={e => setName(e.target.value)} required className="w-full h-12 px-4 bg-slate-50 border border-slate-300 rounded-lg focus:ring-2 focus:ring-primary font-body-md transition-all outline-none" placeholder="أدخل اسم السائق" type="text" />
                        </div>
                        ...
                        <div className="space-y-2">
                            <label className="font-label-md text-on-surface block">رقم لوحة المركبة</label>
                        ...
                        <div className="space-y-2">
                            ...
                            <input value={vehicleDetails} onChange={e => setVehicleDetails(e.target.value)} className="w-full h-12 px-4 bg-slate-50 border border-slate-300 rounded-lg focus:ring-2 focus:ring-primary font-body-md transition-all outline-none" placeholder="مثال: نقل جامبو - نوع السيارة" type="text" />
                        </div>
                ...
                <div className="px-8 py-6 bg-slate-50 border-t border-slate-100 flex items-center justify-end gap-4">
                    ...
                    <button type="submit" form="add-driver-form" disabled={loading} className="px-8 py-2.5 rounded-lg font-label-md bg-primary text-white shadow-lg hover:bg-blue-800 transition-all flex items-center gap-2 disabled:opacity-50">
