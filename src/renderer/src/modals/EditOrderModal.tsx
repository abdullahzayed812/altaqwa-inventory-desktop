...
import { getCustomers, getProducts, updateOrder } from '../api';
...
export const EditOrderModal: React.FC<EditOrderModalProps> = ({ isOpen, order, onClose, onSuccess }) => {
    ...
    useEffect(() => {
        ...
        Promise.all([getCustomers(), getProducts()])
            .then(([c, p]) => {
                ...
                setProducts(p);
                setSelectedCustomer(c.find((cust: any) => cust.id === order.customerId) || null);
                ...
                setNaulonUncollected(String(order.naulonUncollected || 0));
            })
            .catch(console.error);
    }, [isOpen, order]);
            ...
            <div className="bg-white w-full max-w-4xl rounded-2xl shadow-2xl flex flex-col max-h-[92vh]">
                ...
                <div className="px-8 py-5 border-b border-slate-100 flex items-center justify-between bg-white sticky top-0 z-10 rounded-t-2xl">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-slate-900 font-bold text-lg">تعديل الطلب #{order.orderNumber}</h2>
                            <p className="text-sm text-slate-500">تعديل العميل والمنتجات</p>
                        </div>
                    </div>
                ...
                <div className="px-8 py-5 border-t border-slate-100 bg-slate-50/50 rounded-b-2xl flex items-center justify-between">
                    ...
                    >
                        ...
                        {loading ? 'جاري الحفظ...' : `حفظ التعديلات • ${totalAmount.toLocaleString()} ج.م`}
                    </button>
                </div>
            </div>
        </div>
